...
function parsePaths(projectRoot, mode, nativeAppManifest) {
 ...
 * @category env
 */
function getPaths(projectRoot, mode) {
    const configMode = getConfigMode_1.getConfigMode(getMode_1.default({ mode }));
 ...
 * @category env
 */
function getPathsAsync(projectRoot, mode) {
    return __awaiter(this, void 0, void 0, function* () {
        const configMode = getConfigMode_1.getConfigMode(getMode_1.default({ mode }));
        let exp;
        try {
            exp = config_1.getConfig(projectRoot, { skipSDKVersionRequirement: true, mode: configMode }).exp;
        }
        catch (error) { }
        return parsePaths(projectRoot, configMode, exp);
    });
 ...
 * @category env
 */
function getServedPath(projectRoot, mode) {
    const configMode = getConfigMode_1.getConfigMode(getMode_1.default({ mode }));
 ...
 * @category env
 */
function getProductionPath(projectRoot, mode) {
    const configMode = getConfigMode_1.getConfigMode(getMode_1.default({ mode }));
    const { exp } = config_1.getConfig(projectRoot, {
        skipSDKVersionRequirement: true,
        mode: configMode,
