...
async function extractTemplateAppAsyncImpl(templateSpec, targetPath, config, tarStream) {
...
async function installDependenciesAsync(projectRoot, packageManager) {
  ...
  if (packageManager === 'yarn') {
    ...
    if (_semver().default.satisfies(version, '>=2.0.0-rc.24') && nodeLinker !== 'node-modules') {
      ...
      _Logger().default.global.warn(`Yarn v${version} detected, enabling experimental Yarn v2 support using the node-modules plugin.`);

      _Logger().default.global.info(`Writing ${yarnRc}...`);
...
async function getPublishInfoAsync(root) {
  ...
  const {
    exp
  } = await (0, _Config().getProjectConfigAsync)(root, {
