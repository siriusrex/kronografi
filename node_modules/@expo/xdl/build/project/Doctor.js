...
function _Config() {
  const data = _interopRequireWildcard(require("../Config"));
...
async function validateWithSchemaFileAsync(projectRoot, schemaPath) {
  let {
    exp
  } = await (0, _Config().getProjectConfigAsync)(projectRoot, {
...
async function validateAsync(projectRoot, allowNetwork) {
  ...
  try {
    const config = await (0, _Config().getProjectConfigAsync)(projectRoot, {
      skipSDKVersionRequirement: false
    });
    exp = config.exp;
    pkg = config.pkg;
