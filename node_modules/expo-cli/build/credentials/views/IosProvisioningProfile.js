...
async function validateProfileWithoutApple(provisioningProfile) {
  ...
  if (_appleApi().ProvisioningProfileManager.isExpired(base64EncodedProfile)) {
...
async function useProvisioningProfileFromParams(ctx, appCredentials, teamId, provisioningProfile) {
  const {
    experienceName,
    bundleIdentifier
  } = appCredentials;
